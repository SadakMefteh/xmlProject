...
if (isset($_POST['submit'])) {
    ...
    if (empty($login) || empty($password)) {
        ...
        $xml = @simplexml_load_string($results); // Utiliser @ pour éviter les erreurs de parsing visibles

        if ($xml && count($xml->utilisateur) > 0) {
            foreach ($xml->utilisateur as $utilisateur) {
                // Obtenir le privilège utilisateur
                $privilege = (string)$utilisateur->privilege;

                // Redirection basée sur le privilège
                if ($privilege == "Participant") {
                    header("Location: participant.php?&login=" . urlencode($login));
                    exit(); 
                } elseif ($privilege == "Organisateur") {
                    header("Location: organisateur.php?&login=" . urlencode($login));
                    exit(); 
                } else {
                    $errorMessage = "Privilège utilisateur inconnu.";
                    break; // Sortir de la boucle en cas de problème
                }
            }
        } else {
